...
\noindent
Before deeper introduction, we will briefly describe a few concepts that we will involve later so that one could understand the method easier.
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.4\textwidth]{Alpha_AminoAcidball.png}\\
...
\noindent
Among these three steps, the most important one is the peptide separation in the column. Retention time (RT) is the amount of time that a peptide needs to go through the column. Each kind of peptide has a certain amount of retention time based on its hydrophobicity. Therefore, it is a useful feature in peptide separation and identification. It provides us with a new perspective to identify the protein. Besides, it may be used as a important parameter in other experiments involving chemometric analyses of peptide mixtures as well. Therefore, precise prediction of the peptide retention time could be helpful to the protein identification.\\
...
\noindent
What's more, the main algorithm behind current RT prediction methods is mostly based on either Support Vector Regression (SVR) or Artificial Neural Network (ANN). However, all of these methods of SVR and ANN have their own drawbacks and still needs to be improved.\\
...
\fontsize{12pt}{16pt}\selectfont
\noindent
Prediction of the retention time for a given peptide could help to improve the confidence of peptide identification. If the prediction of retention time of a peptide is precise enough, then we could tell which peptide it is or which species does it belong to by cross-matching its predicted retention time with our database. Therefore, developing efficient algorithms and establishing proper models to generate the prediction of peptide has become more important and necessary.\\
