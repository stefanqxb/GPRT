...
\todo[inline]{Don't start with background. Start with introduction. Don't go in any details in the introduction just try to explain the problem with as simple terminologies as possible.}

\textcolor{red}{Re-write the introduction focusing on these things :}

\begin{itemize}
	\item \textcolor{red}{Functions of different biological organisms is determined by the proteins presented in them}
	\item \textcolor{red}{For this reason it is important for us to what proteins are present in them}
	\item \textcolor{red}{Tandem Mass Spectrometry is the leading technique for global and large-scale identification of proteins in complex mixtures.}
	\item \textcolor{red}{Briefly explain how it works, I will send you a sample that I had written a while back}
	\item \textcolor{red}{Talk about the differences between DDA and DIA}
	\item \textcolor{red}{Finally talk about why RT prediction can improve the identification}
\end{itemize}


\section{Background}
Protein, the most essential component of all the cells and organisms, plays an irreplaceable role in people's daily lives including providing energy, strengthening the immune system and catalyzing metabolic reactions within the body. Therefore, the study of \textcolor{red}{the properties of protein} \todo{I don't think that this is correct} is very necessary and considered as one of most important subject in the biological field.\\


\todo[inline]{The goal is identification of proteins and not study of their properties}


\noindent
Before introducing \todo{Remove these from here.} the background, we will briefly describe a few concepts that we will involve later so that one could understand the method easier.
\todo[inline]{Move this list to the background section talking about the biology problem.}
\begin{itemize}
...
\todo[inline]{Don't remove the indentation.}
\todo[inline]{Your problem statement is prediction of retention time and not GP. GP is a tool we are using to solve the problem.}

\textcolor{red}{Re-write this focusing on the following points:}
\begin{itemize}
	\item \textcolor{red}{What is the problem? In more details.}
	\item \textcolor{red}{How people have been thinking about this problem?}
	\item \textcolor{red}{What are some previous attempts in solving the problem?}
	\item \textcolor{red}{How are we attacking this problem? What is the difference? What are the challenges?}
\end{itemize}

\noindent
All of these methods of SVR and ANN have their own drawbacks and still needs to be improved. One alternative option is to implement the Gaussian Processes (GPs) to build the predictive model. GPs provides a flexible framework for probabilistic regression and are widely used to solve the high-dimensional, small-sample or nonlinear problems. \textcolor{red}{Yet none of the existing publications has the prediction of retention times for peptides analyzed with the use of Gaussian Process.}\todo{Remove this}\\

\todo[inline]{After describing the problem, you can say that we want to use GP for solving it since it is a very flexible framework and has not been previously applied to this problem. Then, slightly talk about the challenges of applying GP to this problem.}
...
\todo[inline]{You should talk about features before talking about the SVM or GP. Keep the order of things in mind. You want to control what the reader is thinking. Previously, we you talk about GP, the reader will ask how do we give peptides to GP or SVM? But if you talk about the features first, by just saying we have methods that maps peptides into a vector space then things becomes more clear.}
