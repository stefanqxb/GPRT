...
\begin{abstract}
  The Shotgun Proteomics is the leading technique for protein identification. However, this process produces a large amount of data need to be generated which results in a extremely high computational cost. Retention time (RT) is an important factor to be used to enhance the efficiency of protein identification. By predicting the retention time successfully, we could decrease the computational cost dramatically. This thesis presents a machine learning method, Gaussian Processes, to predict the retention time of a set of peptide in hand. We also implement a feature extraction method called BOW to generate the features for training the model. In addition, we also investigate the effect of different types of optimization methods to the model's parameters. The results show comparable precision of the prediction and relatively low computational cost when comparing with the state-of-art prediction model.
\end{abstract}
...
\section{Features}
\noindent
The effect of an individual amino acid was firstly taken into account when building the model because evidence has implied that the inclusion of this type of additional information could increase the confidence of peptide identification \cite{foo1}. Since peptides are composed of amino acids, it can increase the performance the chromatographic behavior \cite{foo7}. A set of retention time coefficients were generated from only different amino-acid composition by using iterative regression methods \cite{foo8} \cite{foo9} \cite{foo10}. To be specific, the regression model was trained by amino-acid with different compositions and their corresponding retention time. As a result, the model gives a set of coefficient regarding the amino-acid composition and use them to represent the peptides. \\
...
\noindent
where $x$ is the input vector, $w$ is the parameters of the model. The function value is represented by $ f(\mathbf{x})$ and a bias (noise) is included. We assume that the $\epsilon$ is independent identically distributed Gaussian noise with zero mean and variance $\sigma_{n} ^{2}$.
...
p(y|\mathbf{x},\mathbf{w}) = \prod_{i=1}^{n}p(y_{i}|x_{i},\mathbf{w}) = \prod_{i=1}^{n}\frac{1}{\sqrt{2\pi}\sigma_{n}}exp(-\frac{(y_{i}-x_{i}^{T}\mathbf{w})^{2}}{2\sigma_{n}^{2}})
\end{equation}

\noindent
The idea of Bayesian linear model is mainly based on the posterior distribution computed by Bayes' rule:
\begin{equation} \label{eq:5}
p(\mathbf{w}|y,\mathbf{x}) = \frac{p(y|\mathbf{x},\mathbf{w})p(\mathbf{w})}{p(y|\mathbf{x})}
\end{equation}
...
\noindent
Another essential part is the marginal likelihood function $p(y|\mathbf{x})$. The marginal likelihood,(normalizing constant), gives the likelihood of the output by considering only one specific kind of input and is independent of the other factors. To be specific, the marginal likelihood of expected output here only consider the effect of the input $\mathbf{x}$ without taking the effect of the weights $\mathbf{w}$ into account. Therefore, it is given by
\begin{equation} \label{eq:7}
p(y|\mathbf{x}) = \int p(y|\mathbf{x},\mathbf{w})p(\mathbf{w})dw
\end{equation}
...
p(\mathbf{w}|y,\mathbf{x}) = \frac{p(y|\mathbf{x},\mathbf{w})p(\mathbf{w})}{p(y|\mathbf{x})} \sim exp(-\frac{1}{2}(\mathbf{w}-\bar w)^{T}(\frac{1}{\sigma_{n}^{2}}\mathbf{x}\mathbf{x}^{T}+\Sigma^{-1})(\mathbf{w}-\bar w))
\end{equation}
...
\begin{align}\label{eq.9}
    &\bar w= \sigma_{n}^{-2}(\frac{1}{\sigma_{n}^{2}}\mathbf{x}\mathbf{x}^{T}+\Sigma^{-1})^{-1}\mathbf{x}y\notag \\
    &\Sigma_{new} = \frac{1}{\sigma_{n}^{2}}\mathbf{x}\mathbf{x}^{T}+\Sigma^{-1}
\end{align}

\noindent
For Bayesian linear regression, the prediction is made by all possible parameters along with corresponding posterior probability. Thus, we could conclude the predictive jointly distribution of the output $f(x_{new})$ of a new data point $x_{new}$ based on the training dataset $\mathbf{x}$ along with their weights $\mathbf{w}$ and their outputs $y$ as follows :
\begin{align} \label{eq:10}
p(f(x_{new})|x_{new},y,\mathbf{x}) = \int p(f(x_{new})|x_{new},\mathbf{w})p(\mathbf{w}|\mathbf{x},y)dw \notag \\ \sim N(\frac{1}{\sigma_{n}^{2}}x_{new}^{T}(\sigma_{n}^{2}\mathbf{x}\mathbf{x}^{T}+\Sigma^{-1})^{-1}\mathbf{x}y,x_{new}^{T}(\sigma_{n}^{2}\mathbf{x}\mathbf{x}^{T}+\Sigma^{-1})^{-1}x_{new})
\end{align}
...
\noindent
where the $k(\mathbf{x}_{a},\mathbf{x}_{b})$ is a general form of kernel function and $\delta_{ab}$ is a Kronecker delta which is one iff $a\equiv b$ and zero otherwise.\\
...
\begin{equation}\label{eq:16}
   ...
   \left(
    \begin{array}{cc}
      0 ,& \left[
           \begin{array}{cc}
            k(\mathbf{x},\mathbf{x})+\sigma_{n}^{2}I & k(\mathbf{x},x_{new})\\
            k(x_{new},\mathbf{x}) & k(x_{new},x_{new})\\
           \end{array}
          \right]
    \end{array}
   \right)
...
\begin{align}\label{eq:17}
   f_{new}|\mathbf{x},y,x_{new} &\sim N(\bar f(x_{new}), cov(f(x_{new}))), \quad where\\
   \bar f(x_{new}) &= k(x_{new},\mathbf{x})[k(\mathbf{x},\mathbf{x})+\sigma_{n}^{2}I]^{-1}y\\
   cov(f(x_{new})) &= k(x_{new},x_{new})-k(x_{new},\mathbf{x})[k(\mathbf{x},\mathbf{x})+\sigma_{n}^{2}I]^{-1}k(\mathbf{x},x_{new})
\end{align}
