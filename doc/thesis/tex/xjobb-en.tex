...
Protein, the most essential component of all the cells and organisms, plays an irreplaceable role in people's daily lives including providing energy, strengthening the immune system and catalyzing metabolic reactions within the body. Functions of different biological organisms are determined by the proteins presented in them\todo{proteins that are present in them.}. For this reason, it is important and necessary \textcolor{red}{for us}\todo{remove} to know what proteins are in \textcolor{red}{them}\todo{are in different organisms}. As a result, it\todo{this} leads us to \todo{the} problem of \textcolor{red}{the} \todo{remove} large-scale identification of proteins in complex mixtures.
...
\end{figure}
\noindent
Shotgun proteomics \cite{fooa}\cite{foob} is the most popular technique \textcolor{red}{to identify}\todo{for identifying} \textcolor{red}{the}\todo{remove} protein\textcolor{red}{s} in complex mixtures. The main part of this technique is \textcolor{red}{the}\todo{remove} high performance liquid chromatography (HPLC). \textcolor{red}{The}\todo{remove} HPLC has three main steps, firstly, the protein in the mixture of interest will be broken into peptides. A peptide is a short sequence (chain) of amino acid as in Fig.\ref{fig:Fig3}. Then the resulting peptides are separated according to their hydrophobicity on a thin tube called liquid chromatography (LC) column. To be more specific, a specific solvent will be pumped through the column with increasing concentration.

\todo[inline]{Talk about peptides gradually get separated from the pool. This gradual separation is the retention time data. Actually your task is to predict when a peptide is seperated from the pool.}

In the end, those peptide will eluted from the column and ionized by electrospray ionization and then tossed into mass spectrometer. The result\textcolor{red}{s are}\todo{is} a set of spectra\textcolor{red}{ which each of the}\todo{. Each}spectra corresponds to only one or at most a few specific peptides. The fig.\ref{fig:HPLC} shows the general process of HPLC method.\\
...
\todo[inline]{We should re-write the following two paragraphs. You can merge them into one paragraph. You can follow this : (a) Our task is to determine which peptides were in the sample (b) We usually have a list of theoretical peptides that we know should be in the sample (c) Currently, we have to compare every peptide with every spectrum to see if the peptide exists in the pool (d) if the number of peptides is large then this problem has a high complexity (e) Alternatively, we can significantly reduce the number of peptides compared to a spectrum if we know which peptides are more likely to be separated from the pool at the time the spectrum is produced. (f) This time is referred as retention time (RT) (g) Our task is to find a mapping between the peptides and the retention time.}
...
\noindent
\todo[inline]{Different models have been applied to the task of predicting the retention time of the peptides. For example, ...}
In order to predict the retention time \textcolor{red}{of large-scale of}\todo{remove, retention time of the peptides} peptides, an extensive amount of models have been conducted. For example, the Sequence Specific Retention Calculator (SSRC) which predicts retention times based on amino acid sequences is available on-line.\todo{cite} \\

\todo[inline]{Different models have been applied to the task of predicting the retention time of the peptides. For example, ...}

\noindent

\todo[inline]{Re-write the following paragraph! (a) Most methods are based on biological features, (b) we are interested in BOW features that have shown to be successful for sequence analysis.}
To train a regression model, we first need some proper features which can represent the peptide \todo{in a vector space}. Since most of \textcolor{red}{the features now}\todo{most of the available features} are biochemical features, some features with less biochemical meaning but have been proved to be useful in other fields have not been tested to address this biochemical problem so far. Therefore, this thesis will strive to find some new features to represent the peptide for training model and compare their performance to the traditional biochemical features.\\

\todo[inline]{You shouldn't write the GP, the SVM, the ANN. It is like calling me the heydar. Also all these are singular, so no GPs.}

\noindent
\textcolor{red}{What's more,}\todo{remove} the main algorithm behind current RT prediction methods is mostly based on either Support Vector Regression (SVR) or Artificial Neural Network (ANN) and all of these methods have their own drawbacks that need to be improved.\\

\noindent
\textcolor{red}{Therefore,}\todo{remove} in this thesis, we would like \textcolor{red}{to implement}\todo{apply} the Gaussian Processes (GPs) model to predict the retention time and investigate its performance by comparing \todo{it} with the other existing methods.\\

\noindent
\textcolor{red}{The GPs}\todo{GP} provides a flexible framework for probabilistic regression and \textcolor{red}{are}\todo{is} widely used to solve \textcolor{red}{the}\todo{remove} high-dimensional, small-sample or nonlinear problems\todo{You can write this a bit better}. Yet none of the existing publications has \todo{used this model for predicting ...} the prediction of retention times for peptides analyzed with the use of Gaussian Process.\\


\noindent
In addition, since \textcolor{red}{the}\todo{remove} GP model has some parameters need to be optimized \textcolor{red}{and these parameters could}\todo{which have ..} have a significant impact on the prediction accuracy\textcolor{red}{,}.\todo{In this thesis, we will investigate different optimization methods and analyze their properties.} different optimization methods towards these parameters will be implemented and their properties will be analyzed in this thesis too.


\section{Outline}

\todo[inline]{Write the outline this way, Chapter 2, introduces the related works .... Chapter 3, provides a detailed description of Gaussian process ...}

\noindent
The structure of the thesis is as follow\textcolor{red}{a,}\todo{following:} we will first introduce the related work on retention time prediction in chapter 2. It will be followed by the methodologies we use in in chapter 3. Within chapter 3, the detailed description of Gaussian Processes and a detailed mathematics derivation will be covered in section 1. Following that, section 2 introduces a different method regarding features selection of Gaussian Processes. The optimization will be covered in section 3 and two different method will be used to optimize the parameters of our model. In chapter 4, the experimental results and evaluation will be analyzed and our model will be compared with some other popular predictors. A discussion of the limitation of our model will be given in chapter 5. Finally, we will discuss the future work and give some perspectives for it in chapter 6.
