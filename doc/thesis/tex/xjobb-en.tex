...
\noindent
where $m$ is the weight between two kernel functions. We can see that if $ m = 0$, the multi-kernel is actually a RBF kernel and it is a polynomial kernel when $m = 1$. By combining two kernels as a new one and finding a proper weight between them, we could achieve our goal. As for how to find the proper weight factor $m$, we will explain in the section of optimization. The performance of this multi-kernel function will also be evaluated and compared to the single kernel function later.
...
\begin{tabular}{l|cc|cc}
         ...
         & $\rho$ & $\Delta_{t95\%}$ & $\rho$ & $\Delta_{t95\%}$ \\
        \hline
         BOW feature & 0.9724 & 0.1822 &  0.9655 & 0.2095\\
        \hline
         Elude feature  & 0.9683 & 0.1718 &  0.9701 & 0.1809\\
        \hline
\end{tabular}
...
\begin{tabular}{l|cc|cc}
   ...
   & $\rho$ & $\Delta_{t95\%}$ & $\rho$ & $\Delta_{t95\%}$ \\
\hline
 BOW  & 0.9751 & 0.1731 & 0.9744 & 0.1731 \\
\hline
 Elude & 0.9744 & 0.1537 & 0.9757 & 0.1537 \\
\hline
\end{tabular}
\caption{Comparison of performance before and after PSO for GP and SVR model}
\label{table4}
...
\section{Conclusion}
The retention time of peptide is a piece of important and useful information for many biochemical problems. For example, the retention time can be used in in some quality control experiments as the control parameter. It can also be used for the identification of a new peptide which is alternative way to identify it instead of calculating the peptide＊s spectrum. Therefore, the successful prediction of retention time could be really useful in many aspects.\\
...
\noindent
However, we should see that the prediction of peptide's retention time is just another example of implementation of machine learning method in biochemical problem. From a long-term eyesight, with the help of computer science and machine learning theories, more and more advanced machine learning methods will be utilized to solve the biochemical problems and with the development of biotechnology, more and more biochemical problems will arise which will also motivate the development of machine learning methods. The connection between machine learning techniques and biotechnology will become increasing strong in the further.
