...
\begin{abstract}
  \textcolor{red}{The}\todo{remove} Shotgun Proteomics is the leading technique for protein identification \todo{in complex mixtures}. However, \textcolor{red}{this process produces a large amount of data need to be generated which results in a extremely high computational cost.}\todo{Re-write this sentence} Retention time (RT) is an important factor to be used to enhance the efficiency of protein identification. By predicting the retention time successfully, we could decrease the computational cost dramatically. This thesis presents a machine learning method, Gaussian Processes \todo{You are not introducing Gaussian Process, you are using it to solve this problem}, to predict the retention time of a set of peptide in hand. We also implement a feature extraction method called BOW\todo{Bag-of-Words} to generate the features for training the model. In addition, we also investigate the effect of different types of optimization methods to the model's parameters. The results show comparable precision of the prediction and relatively low time cost when comparing with the state-of-art prediction model.
\end{abstract}
