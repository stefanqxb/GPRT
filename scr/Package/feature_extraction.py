...
import copy_retention_model as crm
...
class peptide:
    ...
    def elude_descriptor(self, aaAlphabet, psmDescriptions):
        peptide = self.sequence
        normalizeRetentionTimes = True
        customIndex = crm.buildRetentionIndex(aaAlphabet, psmDescriptions, normalizeRetentionTimes)
        customIndex =  dict(zip(aaAlphabet, customIndex))
        featureVector = crm.computeRetentionFeatureMatrix(aaAlphabet, peptide, customIndex)
        self.elude_descriptor = featureVector
